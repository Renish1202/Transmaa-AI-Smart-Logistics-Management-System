...
def _load_kb_chunks() -> List[Tuple[str, str, str]]:
...
def _build_index() -> List[RagChunk]:
    ...
    try:
        with open(RAG_INDEX_PATH, "w", encoding="utf-8") as handle:
            json.dump([chunk.__dict__ for chunk in rag_chunks], handle)
    except OSError:
        pass
...
def _load_index_from_disk() -> List[RagChunk]:
    try:
...
def _index_is_stale(kb_files: List[str]) -> bool:
    if RAG_FORCE_REINDEX:
        return True
    if not os.path.exists(RAG_INDEX_PATH):
        return True
    kb_mtime = _latest_mtime(kb_files)
    try:
        index_mtime = os.path.getmtime(RAG_INDEX_PATH)
    except OSError:
        return True
...
def get_index() -> List[RagChunk]:
    ...
    with _index_lock:
        if _cached_index is not None and _cached_index_mtime >= kb_mtime and not RAG_FORCE_REINDEX:
            return _cached_index

        if _index_is_stale(kb_files):
            _cached_index = _build_index()
        else:
            _cached_index = _load_index_from_disk()

        _cached_index_mtime = kb_mtime
        return _cached_index
